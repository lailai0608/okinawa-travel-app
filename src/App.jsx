  ...
  const itinerary = [
    ...
    { 
      ...
      items: [
        ...
        { type: '🥞', time: '16:00', title: '瀨長島: 幸福鬆餅', nav: 'Happy Pancake Okinawa', tag: '重要預約', note: '預約 16:00 幸福鬆餅' }
      ]
    }
  ];
  ...
  return (
    <div className="max-w-md mx-auto bg-slate-50 min-h-screen pb-24 font-sans text-slate-800">
      <header className="bg-white p-6 border-b border-slate-100 sticky top-0 z-50 shadow-sm">
        ...
        <p className="text-[10px] text-slate-400 font-bold uppercase mt-1">Personal Travel Assistant</p>
      </header>
      ...
      {activeTab === 'itinerary' ? (
              ...
              {day.items.map((item, i) => (
                <div key={i} className="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm">
                  <div className="flex justify-between items-center mb-3">
                    <span className="text-[10px] bg-slate-100 text-slate-500 px-2 py-1 rounded font-mono font-bold">{item.time}</span>
        ...
        <div className="p-8 space-y-6">
          <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
            <h2 className="font-bold text-slate-800 flex items-center gap-2 mb-4"><Info size={18} className="text-blue-500" /> 緊急聯絡</h2>
            <div className="space-y-2 text-sm text-slate-600">
              <p className="flex justify-between"><span>救護車/火警</span><span className="text-red-500 font-bold">119</span></p>
              <p className="flex justify-between"><span>警察局</span><span className="text-blue-500 font-bold">110</span></p>
            </div>
          </div>
        </div>
      )}

      <nav className="fixed bottom-0 left-0 right-0 max-w-md mx-auto bg-white/90 backdrop-blur-md border-t border-slate-100 flex justify-around py-4 shadow-2xl">
        <button onClick={() => setActiveTab('itinerary')} className={`flex flex-col items-center ${activeTab === 'itinerary' ? 'text-blue-600' : 'text-slate-300'}`}>
          <Calendar size={20} /><span className="text-[9px] font-black mt-1 uppercase">行程</span>
        </button>
        <button onClick={() => setActiveTab('tools')} className={`flex flex-col items-center ${activeTab === 'tools' ? 'text-blue-600' : 'text-slate-300'}`}>
          <CreditCard size={20} /><span className="text-[9px] font-black mt-1 uppercase">工具</span>
        </button>
      </nav>
    </div>
  );
